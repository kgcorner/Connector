...
export class FundService {
  ...
  submitRequest(message, fundOnBill, document) {
    let formData = new FormData();
    formData.append('message', message);
    formData.append('fundOnBill', fundOnBill);
    formData.append('document', document);
    return this.doPostFile(environment.serverUrl+this.submit_fundRequest_url, null, formData);
  }

  approveRequest(documentId, totalApproved) {
    let formData = new FormData();
    formData.append('id', documentId);
    formData.append('totalApproved', totalApproved);
